...
std::vector<double> PSOOptimizer::optimize(const OptimizationProblem& problem) {
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        if (logger_) logger_->logIteration(iter, gbest_val, 0.0);
    }
    ...
    for (size_t i = 0; i < N; ++i)
        x_c[i] = std::complex<double>(gbest[i], 0.0);
    double final_loss = problem.loss->evaluate(problem.model->apply(gbest), problem.target) +
        problem.reg_weight * RegLoader::instance().eval_func(problem.reg_handle, x_c.data(), N);
    if (logger_) logger_->logFinalResult(gbest, final_loss, max_iter, "completed");
    return gbest;
}
